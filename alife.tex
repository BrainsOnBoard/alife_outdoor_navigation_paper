...
As these algorithms have not yet been tested in the real world, in this paper we show that an ANN with an Infomax learning rule is capable of delivering reliable direction information along outdoor routes, even when scenes contain few local landmarks and high-levels of noise (from changing lighting conditions, uneven terrain etc) with all computation performed on a small low-power robot.
...
While our ANN-based algorithms have been extensively tested in simulations of ant habitats, they have not yet been tested in the real world.  
In this paper, we therefore show that a single layer ANN can autonomously guide a robot through outdoor routes of up to \SI{10}{\metre} with all computation performed on-board.
...
\begin{figure}[t]
    ...
    \textbf{B}~Difference image with visual aliased route image found at location marked with pink arrow in Figure~\ref{fig:vector_fields}c.
...
We trained both the Infomax and Perfect Memory algorithms on the images taken along the six routes shown in Figure~\ref{fig:vector_fields}a and then used them to find the direction a robot would move when placed at each grid point within \SI{4}{\metre} of the trained route. 
...
This introduces sufficient difference between the grid image and the correct route image (Figure~\ref{fig:aliasing}c) that the Perfect Memory model matches the more distant (but still nearby) image better (possibly as the paving slab has a similar pitch to the grid point). 
Thus the direction taken is similar to the direction taken at this point in the route which results in an error at this location (indicated with a pink arrow in Figure~\ref{fig:vector_fields}c).
...
After applying sky-segmentation to the input images and only scanning \SI{\pm 90}{\degree} away from the direction of the nearest section of the route, the Infomax ANN results in a lower median error than the Perfect Memory control algorithm in 3 of the 6 routes although this difference is not statistically significant ($p=0.84$, paired, two-sided Wilcoxon signed rank test, $n=6$).
Furthermore, if we compare Figures~\ref{fig:vector_fields}d~and~\ref{fig:vector_fields}e, the vector field plotted using the Infomax ANN appears much smoother, suggesting that rather than losing useful information, the Infomax encoding may actually be beneficial for smoothing out overly tortuous training routes.
...
Additionally, Infomax implements a form of decorrelation between inputs and outputs, a process which has been widely reported in the brain\todo{REF}.
Furthermore, local variants of Infomax have been demonstrated within recurrent spiking neural networks~\citep{Hayakawa2014} and future work will explore the use of such biologically realistic implementations for navigation.
