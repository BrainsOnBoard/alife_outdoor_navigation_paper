...
\usepackage{natbib,alifeconf,amsmath,listings,subcaption,textcomp,url,xcolor}  %% The order is important - natbib must be before alifeconf
...
\author{James C. Knight$^{1}$, Daniil Sakhapov$^{2}$, Norbert Domcsek$^{1}$, Alex Dewar$^{1}$, Thomas Nowotny$^{1}$ \and Andrew Philippides$^{1}$ \\
\mbox{}\\
...
Finally, rather than this compact representation necessarily losing information, there are instances where the use of an ANN ameliorates the problems of sub-optimal paths caused by very tortuous/kinked training routes. 
...
Visual homing -- the ability to navigate back to a place of interest using visual information alone -- is a problem of great interest for both engineers seeking to build autonomous robots and neuroethologists seeking to understand its neural basis in animals~\todo{cite Andy and Paul?}.
...
We have shown that route knowledge can be learnt and represented holistically using an artificial neural network (ANN)~\todo{cite andy}, without specifying when or what to learn~\citep{Baddeley2011} and from a single exposure to the route data~\citep{Baddeley2012}. 
...
Firstly, if an agent stores a view when facing a given direction, the difference between it and views from a nearby location, but at random headings, will be minimised when the agent is facing the same direction as when the first image was stored~\citep{Zeil2003}. 
...
This results in a rotational familiarity function (RFF -- Figure~\ref{fig:good_ridf}) which, for each heading faced, gives a familiarity score with the heading with the highest score being the most familiar and the movement direction taken. 
...
%
\begin{align}
    IDF\Bigl(C(\vec{x}, \theta), S(\vec{y}, \phi)\Bigr) = \frac{1}{p \times q} \sum\limits_{i=1}^p{\sum\limits_{j=1}^q|{C_{i,j} - S_{i,j}|}}
\end{align}
%
where $C(\vec{x}, \theta)$ is a $p \times q$ pixel view captured at location $x$ and heading $\theta$; $S(\vec{y}, \phi)$ is a $p \times q$ pixel snapshot stored in memory; and $C_{i,j}$ and $S_{i,j}$ refers to the intensity of pixels in row $i$ and column $j$ of the captured view and stored snapshot respectively.
...
\end{align}
%
where $x_{j}=C_{j / q, j \bmod q}$, the value of the $jth$ input and $N=p \times q$~(the number of input units).
...
%
\begin{align}
    d(\vec{x})=\sum\limits_{i=1}^N |h_{i}|     \label{eqn:infomax_response}
\end{align}
%
where $||$ denotes the absolute value.
By rotating $C$ through a range of $\theta$, an RFF can be calculated using the ANN.

The difference between the way the perfect memory and ANN represent familiarity will be subtle. 
...
\todo{something about route smoothing}
...
%Second, in terms of training time, using a route not a map, requires one single traversal through the world, and our Infomax implementation requires one set of weight updates per image used, meaning training time scales linearly with route length and, could in principle be performed in real time [could it???]. 
%This allows rapid learning of the world, as is seen in the one-trial learning of ants [need a ref from Paul] and compares favourably with SLAM [I’m being purposefully vague!]. 
...
%To put this into context of robotic control algorithms, we note that recent SLAM implementations such as FLaME~\citep{Greene2017} have been shown to run on autonomous quadrotors in only around \SI{10}{\milli\second} per frame. 
